...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth]{figs/snatchapp_screenshot.png}
...
\end{lstlisting}
In \texttt{esp32\_airtag\_mac\_rotation/main/openhaystack\_main.c}, set
\texttt{DEVICE\_ID} to distinguish co-located mimicked devices and configure the
MAC address rotation period (paper range: 10--60\,s). In
\texttt{esp32\_sound\_maker/main/main.c}, set \texttt{device\_type} and
\texttt{target\_mac} to the target accessory (its MAC can be read with
...
\end{lstlisting}
It band-passes the recording, cross-correlates it with the chirp template to
detect the AirTag sound arrivals, and prints their level ($\approx-42.8$\,dB),
confirming the toolchain is correctly installed. With the prebuilt APK installed,
E0 (below) is the live on-device demo.

\subsection{Evaluation Workflow}
The evaluation comprises one functional check (E0) and seven reproduction
experiments (E1--E7). E1--E7 run from the provided data and need no Apple
hardware; the recommended E0 demo needs only an Android phone with the prebuilt
APK --- its target is the lost Apple devices already broadcasting nearby, or one
you control (an ESP32 board, or an Apple device in the Separated/Lost state). The remaining
aggregate field results (the per-level navigation success rates, paper
Figure~16, and the averaged end-to-end navigation times) require extensive
real-world trials and are out of automated scope; the artifact provides the
implementation and logging to repeat them.
...
\emph{We urge every reviewer to try this on their own phone.} Install the
prebuilt \texttt{SnatchAPP} APK (part~(A)), open the \textit{Scanner}, and watch
it discover the \emph{real} Apple devices already in the Separated (Lost) state
broadcasting around you --- in any public space there are usually several. Then,
...
\emph{(a)}~RSSI decays smoothly while the scanned advertising interval stays
short then rises sharply beyond $\sim$60\,m (discoverable out to tens of metres);
\emph{(b)}~the triggered sound is loud near the device ($\sim$92\,dB) and falls
to ambient by $\sim$30\,m as the trigger time grows --- reproducing
Figures~14--15.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{BLE discovery range: RSSI (left) and scanned advertising interval
    (right) vs distance.}
    \label{sf:maxdist_rssi}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{figs/maxdist_sound.pdf}
    \caption{Sound-trigger range: sound level (left) and connect-and-trigger
    time (right) vs distance.}
...
Example walking trajectories drawn from the end-to-end navigation evaluation
(the academic-building runs Figure~17 averages over), one per attack level.
...
from the end-to-end evaluation. \emph{(a)}~the Level-1 acoustic walk; \emph{(b)}~the
Level-2 RSSI--IMU walk, coloured by the target's RSSI; \emph{(c)}~the Level-3
walk under clustering, where two lost devices rotate their MAC every 30\,s
(colour = RSSI, marker shape = distinct MAC). Each converges on the target
(green Start $\to$ red End). The Level-2 panel reuses the trace from (E4), as
the academic-building Level-2 runs were captured only as screenshots.
